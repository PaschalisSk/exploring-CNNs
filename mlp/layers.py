...
class ConvolutionalLayer(LayerWithParameters):
    ...
    def fprop(self, inputs):
        ...
        """
        # Transform from (2,3,4,4) to (12,18)
        X_col = im2col_indices(inputs, self.kernel_height, self.kernel_width,
                               padding=0)
        # From (2,3,2,2) to (2,12)
        W_row = im2col_indices(self.kernels,
                               self.kernel_height, self.kernel_width,
                               padding=0).T

        # Calculate the (2, 18) result
        res = np.dot(W_row, X_col) + self.biases[:,np.newaxis]

        output_height = self.input_height - self.kernel_height + 1
        output_width = self.input_width - self.kernel_height + 1
        batch_size = inputs.shape[0]

        output_shape = (self.num_output_channels, output_height, output_width, batch_size)
        # From (2,18) to (2,3,3,2) and return the correct shape (2,2,3,3)
        output = res.reshape(output_shape)
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        Returns:
            ...
            (batch_size, num_input_channels, input_height, input_width).
        """
        # From (2,2,3,3) to (2,3,3,2), the opposite of the fprop
        G_reshaped = grads_wrt_outputs.transpose(1, 2, 3, 0)
        # From (2,3,3,2) to (2,18), again the reverse of fprop
        G_reshaped = G_reshaped.reshape(self.num_output_channels, -1)
        # W_col is (12,2)
        W_col = im2col_indices(self.kernels,
                               self.kernel_height, self.kernel_width,
                               padding=0)
        # (12,18) result reshaped to 4D with col2im
        res = np.dot(W_col, G_reshaped)
        out = col2im_indices(res, inputs.shape,
                             self.kernel_height, self.kernel_width, padding=0)
        return out
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        """
        # Same as bprop, G_reshaped is (2,18)
        G_reshaped = grads_wrt_outputs.transpose(1, 2, 3, 0)
        G_reshaped = G_reshaped.reshape(self.num_output_channels, -1)
        # From (2,3,4,4) to (18,12)
        X_row = im2col_indices(inputs, self.kernel_height, self.kernel_width,
                               padding=0).T

        grads_wrt_kernels = np.dot(G_reshaped, X_row)
        # From (2,12) to (2,3,2,2)
        grads_wrt_kernels = grads_wrt_kernels.reshape(self.kernels_shape)
        # The bias gradient is the sum for all the axis except the one
        # representing the kernels(each kernel has the same bias)
...
class MaxPooling2DLayer(Layer):
    ...
    def fprop(self, inputs):
        ...
        # From (2,3,8,8) to (12,32)
        I_cols = im2col_indices(inputs, self.size, self.size,
                                padding=0, stride=self.stride)
        # Make each column have the numbers of each window
        # From (12,32) to (4,96)
        I_cols = I_cols.reshape((self.size**2, -1), order='F')
        # Get the max indexes first as they are used in bprop
        max_idxs = I_cols.argmax(axis=0)
        max_cols = I_cols[max_idxs, np.arange(max_idxs.shape[0])]
        # From (96,1) to (4,4,2,3) and then return the correct axis swap
        # to get the required dimensions
        max_cols = max_cols.reshape(output_height, output_width,
                                    inputs.shape[0], inputs.shape[1])
        self.cache = (I_cols, max_idxs)
        return max_cols.transpose(2, 3, 0, 1)
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        """
        I_cols, max_idxs = self.cache

        # From (2,3,4,4) to (96,1), transpose first to get the same ordering
        # as when we found the max indexes
        G_reshaped = grads_wrt_outputs.transpose(2, 3, 0, 1).flatten()
        # Create zero array to hold results
        grads_wrt_input = np.zeros_like(I_cols)
        col_idxs = np.arange(grads_wrt_input.shape[1])
        # Copy the grads_wrt_outputs to the cells that had the max value in fprop
        grads_wrt_input[max_idxs, col_idxs] = G_reshaped
        # From (4,96) get to (12,32), the reverse of what we did in fprop
        grads_wrt_input = grads_wrt_input.reshape((self.size**2*inputs.shape[1],-1), order='F')
        return col2im_indices(grads_wrt_input, inputs.shape,
                                         self.size, self.size,
                                         padding=0, stride=self.stride)
